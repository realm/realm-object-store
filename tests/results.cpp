...
TEST_CASE("results: distinct") {
    ...
    SECTION("Limit after distinct") {
        ...
        // sort() is only applied to unique
        Results limited = unique.limit(LimitDescriptor(2));
        // limited:
        //  0, Foo_0, 10
        //  1, Foo_1,  9
        REQUIRE(limited.size() == 2);
        REQUIRE(limited.get(0).get<Int>(col_num2) == 10);
        REQUIRE(limited.get(1).get<Int>(col_num2) == 9);
    }
