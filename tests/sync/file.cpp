...
TEST_CASE("sync_file: SyncFileManager APIs", "[sync]") {
    ...
    const std::string manager_path = base_path + "syncmanager/" + expected_clean_app_id + "/";
    ...
    SECTION("user directory APIs") {
        const std::string expected = manager_path + "mongodb-realm/" + identity + "/";
    ...
    SECTION("Realm path APIs") {
        ...
        auto expected_name = manager_path + "mongodb-realm/abcdefghi/realms%3A%2F%2Fr.example.com%2F%7E%2Fmy%2Frealm%2Fpath";
        ...
        SECTION("legacy local identity path is detected and used") {
            ...
            const std::string local_id_expected_name = manager_path + "mongodb-realm/123456789/realms%3A%2F%2Fr.example.com%2F%7E%2Fmy%2Frealm%2Fpath";
            const std::string local_id_expected_name_with_suffix = local_id_expected_name + ".realm";

            util::try_make_dir(manager_path + "mongodb-realm/");
            util::try_make_dir(manager_path + "mongodb-realm/" + local_identity);
