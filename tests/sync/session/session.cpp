...
TEST_CASE("SyncSession: management by SyncUser", "[sync]") {
    ...
    SECTION("a SyncUser can properly retrieve its owned sessions") {
        std::string path_1;
        std::string path_2;
        auto user = SyncManager::shared().get_user({ "user1a", dummy_auth_url }, "not_a_real_token", "not_a_real_token");
    ...
    SECTION("a SyncUser properly unbinds its sessions upon logging out") {
        auto user = SyncManager::shared().get_user({ "user1b", dummy_auth_url }, "not_a_real_token", "not_a_real_token");
    ...
    SECTION("a SyncUser defers binding new sessions until it is logged in") {
        ...
        auto user = SyncManager::shared().get_user({ user_id, dummy_auth_url }, "not_a_real_token", "not_a_real_token");
        user->log_out();
        ...
        user = SyncManager::shared().get_user({ user_id, dummy_auth_url }, "not_a_real_token_either", "not_a_real_token");
        EventLoop::main().run_until([&] { return sessions_are_active(*session1, *session2); });
        REQUIRE(user->all_sessions().size() == 2);
    }
    ...
    SECTION("sessions that were destroyed can be properly recreated when requested again") {
        ...
        auto user = SyncManager::shared().get_user({ "user1e", dummy_auth_url }, "not_a_real_token", "not_a_real_token");
        {
    ...
    SECTION("a user can create multiple sessions for the same URL") {
        auto user = SyncManager::shared().get_user({ "user", dummy_auth_url }, "not_a_real_token", "not_a_real_token");
        auto create_session = [&]() {
...
TEST_CASE("sync: token refreshing", "[sync]") {
    ...
    // Disable file-related functionality and metadata functionality for testing purposes.
    TestSyncManager init_sync_manager;
    auto user = SyncManager::shared().get_user({ "user-token-refreshing", dummy_auth_url }, "not_a_real_token", "not_a_real_token");
...
TEST_CASE("SyncSession: close() API", "[sync]") {
    ...
    auto user = SyncManager::shared().get_user({ "close-api-tests-user", "https://realm.example.org" }, "not_a_real_token", "not_a_real_token");
...
TEST_CASE("SyncSession: update_configuration()", "[sync]") {
    ...
    auto user = SyncManager::shared().get_user({"userid", dummy_auth_url}, "not_a_real_token", "not_a_real_token");
...
TEST_CASE("sync: error handling", "[sync]") {
    ...
    const std::string user_id = "user1d";
    std::string on_disk_path;
    auto user = SyncManager::shared().get_user({ user_id, "https://realm.example.org" }, "not_a_real_token", "not_a_real_token");
