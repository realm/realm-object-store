...
def doWindowsBuild() {
  return {
    node('windows') {
      ...
      sshagent(['realm-ci-ssh']) {
        bat """
          "${tool 'cmake'}" . -DREALM_ENABLE_SYNC=ON -DCMAKE_TOOLCHAIN_FILE=c:\\src\\vcpkg\\scripts\\buildsystems\\vcpkg.cmake -DVCPKG_TARGET_TRIPLET=x86-windows-static -DCMAKE_BUILD_TYPE=Release -DCMAKE_GENERATOR_PLATFORM=Win32
...
def doWindowsUniversalBuild() {
  return {
    node('windows') {
      ...
      sshagent(['realm-ci-ssh']) {
        bat """
          "${tool 'cmake'}" . -DREALM_ENABLE_SYNC=ON -DCMAKE_TOOLCHAIN_FILE=c:\\src\\vcpkg\\scripts\\buildsystems\\vcpkg.cmake -DVCPKG_TARGET_TRIPLET=x86-uwp-static -DCMAKE_SYSTEM_NAME="WindowsStore" -DCMAKE_SYSTEM_VERSION="10.0" -DCMAKE_BUILD_TYPE=Release -DCMAKE_GENERATOR_PLATFORM=Win32
