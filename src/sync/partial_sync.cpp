...
IncludeDescriptor generate_include_from_keypaths(const std::vector<StringData>& paths, SharedRealm realm, const ObjectSchema* object_schema, parser::KeyPathMapping& mapping)
{
    std::string table_name = ObjectStore::table_name_for_object_type(object_schema->name);
    ConstTableRef base_table = realm->read_group().get_table(StringData(table_name));
    REALM_ASSERT(bool(base_table));
    ...
    for (size_t i = 0; i < paths.size(); ++i) {
        ...
        if (keypath.size() == 0) {
            throw InvalidPathError(util::format("missing property name while generating INCLUDE from keypaths"));
        }
