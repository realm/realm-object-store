...
std::vector<SyncUserIdentity> SyncUserMetadata::identities() const
{
    REALM_ASSERT(m_realm);
    m_realm->verify_thread();
    ConstLinkViewRef linklist = m_row.get_linklist(m_schema.idx_identities);
...
void SyncUserMetadata::set_refresh_token(util::Optional<std::string> user_token)
{
    ...
    m_row.set_string(m_schema.idx_refresh_token, user_token.value());
...
void SyncUserMetadata::set_identities(std::vector<SyncUserIdentity> identities)
{
    ...
    auto link_list = m_row.get_linklist(m_schema.idx_identities);
...
void SyncUserMetadata::set_access_token(util::Optional<std::string> user_token)
{
    ...
    m_realm->verify_thread();
    m_realm->begin_transaction();
    m_row.set_string(m_schema.idx_access_token, user_token.value());
    m_realm->commit_transaction();
}
