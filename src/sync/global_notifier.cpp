...
#include "sync/impl/admin_realm_listener.hpp"
...
public:
    ...
    struct RealmToCalculate {
        ...
        RealmToCalculate(sync::ObjectID realm_id, std::string virtual_path)
                : realm_id(realm_id), virtual_path(std::move(virtual_path)) {
        }
    };
...
GlobalNotifier::Impl::Impl(std::unique_ptr<Callback> async_target,
                           std::string local_root_dir, SyncConfig sync_config_template)
        : AdminRealmListener(local_root_dir, std::move(sync_config_template)), m_logger(SyncManager::shared().make_logger()),
...
void GlobalNotifier::Impl::register_realm(sync::ObjectID id, StringData path) {
    ...
    info->coordinator->set_transaction_callback(
            [=, weak_self = std::move(weak_self)](VersionID old_version, VersionID new_version) {
                ...
                if (info->shared_group) {
                    m_logger->trace("Global notifier: sync transaction on (%1): Realm already open",
                                    info->virtual_path);
                } else {
...
GlobalNotifier::GlobalNotifier(std::unique_ptr<Callback> async_target,
    ...
    m_impl->m_signal = std::make_shared<util::EventLoopSignal<Impl::SignalCallback>>(
            Impl::SignalCallback{weak_impl});
}
...
GlobalNotifier::ChangeNotification::ChangeNotification(std::shared_ptr<GlobalNotifier::Impl> notifier,
        ...
        : realm_path(std::move(virtual_path)), type(Type::Change), m_realm_id(realm_id),
          m_config(std::move(config)), m_old_version(old_version), m_new_version(new_version),
...
GlobalNotifier::ChangeNotification::ChangeNotification(std::shared_ptr<GlobalNotifier::Impl> notifier,
                                                       std::string virtual_path,
                                                       sync::ObjectID realm_id)
        : realm_path(std::move(virtual_path)), type(Type::Delete), m_realm_id(realm_id),
          m_notifier(std::move(notifier)) {
}
...
std::unordered_map<std::string, CollectionChangeSet> const&
GlobalNotifier::ChangeNotification::get_changes() const {
